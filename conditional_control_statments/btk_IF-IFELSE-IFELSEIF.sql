-- Conditional Control Statements (IF, IF ELSE, IF ELSIF)


-- IF EXAMPLE 
DECLARE
    birth_day  date := TO_DATE('21/05/1990', 'dd/mm/yyyy');
    age        number;
BEGIN
    age := floor((sysdate - birth_day) / 365);
    
    IF age < 15 THEN 
        DBMS_OUTPUT.PUT_LINE('I am a child and my current age is ' || age);       
    END IF;
END;
...
BEGIN
    ...
    IF age < 15 THEN 
        DBMS_OUTPUT.PUT_LINE('I am a child and my current age is ' || age);
    ELSE
        DBMS_OUTPUT.PUT_LINE('I am NOT a child and my current age is ' || age);        
    END IF;
END;
